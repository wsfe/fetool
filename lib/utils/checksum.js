...
exports.compareChecksum = exports.genChecksumJson = void 0;
...
var getFileChecksum = function getFileChecksum(fileBuffer) {
  return _crypto["default"].createHash('md5').update(fileBuffer).digest('hex');
};
/**
...
var genChecksumByPath = function genChecksumByPath(relativePath, map) {
  try {
    var stat = _fs["default"].statSync(relativePath);

    if (stat.isDirectory()) {
      try {
        var dirList = _fs["default"].readdirSync(relativePath);
      ...
      try {
        var fileBuffer = _fs["default"].readFileSync(relativePath);
