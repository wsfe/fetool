...
exports.default = function (app, options) {
  ...
  return function (req, res, next) {
    ...
    projectCwd = sysPath.join(process.cwd(), projectName),
        // 项目的绝对路径
    project = _services.projectService.getProject(projectCwd, ENV.LOC, true),
...
function watchConfig(projectName, paths, projectCwd) {
  if (!watchCache[projectName]) {
    ...
    watcher.on('change', function () {
      ...
      _services.projectService.deleteProject(projectCwd, ENV.LOC);
    });
...
function setHotEntry(config) {
  Object.keys(config.entry).forEach(function (key) {
    ...
    var hotEntry = whmPath + '?reload=true&path=/__webpack_hmr&timeout=9999999&overlay=false';
...
function singleMode(project, projectName) {
  ...
  if (!middleware) {
    ...
    // };
    if (_args.hot) {
      _app.use((0, _webpackHotMiddleware2.default)(compiler, {
        log: false,
        path: '/__webpack_hmr'
      }));
    }
