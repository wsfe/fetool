...
function () {
  ...
  _createClass(Generator, [{
    ...
    value: function generate(name, src, dest) {
      ...
      metalsmith.use(this.askQuestions(options.prompts)).use(this.filterFiles(options.filters)).use(this.renameFiles(options.renames)).use(this.renderTemplateFiles(options.skipInterpolation)); // 执行metalsmith函数，或者metalsmith after函数
    ...
    key: "renameFiles",
    value: function renameFiles(renames) {
      return function (files, metalsmith, done) {
        if (!renames) {
          return done();
        }
        ...
        var fileNames = Object.keys(files);
        var metalsmithMetadata = metalsmith.metadata();
        Object.keys(renames).forEach(function (glob) {
          fileNames.forEach(function (file) {
            if ((0, _minimatch["default"])(file, glob, {
              dot: true
            })) {
              var rename = renames[glob];
              var renamedEntry = _path["default"].dirname(file) + '/';
              ...
              if (typeof rename === 'function') {
                renamedEntry += rename(_path["default"].basename(file), metalsmithMetadata);
              } else {
                renamedEntry += rename;
              }
