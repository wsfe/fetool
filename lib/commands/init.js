...
var _generate = _interopRequireDefault(require("../models/generate"));
...
function init(program) {
  ...
  program.command('init [projectName]').description('generator a new project from template').option('-t, --type [value]', 'template type').option('-m, --multi', 'a multi-page application').option('-c, --clone', 'use git clone').action(function (rawName, cmd) {
    ...
    function run() {
      ...
      if (multi) type = 'multi';
      var templatePath = "wsfe/fet-templates-".concat(type);
      var tmpPath = path.join(home, ".fet-templates/".concat(type));
      var spinner = ora('download template...');
      spinner.start(); // Remove if local template exists

      if (exists(tmpPath)) rm(tmpPath);
      download(templatePath, tmpPath, {
        clone: clone
      }, function (err) {
        spinner.stop();
        ...
        new _generate["default"]().generate(projectName, tmpPath, to, function (err) {
          if (err) {
            error(err);
            process.exit(1);
          }
