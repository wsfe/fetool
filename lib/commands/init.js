...
var templatePath = 'vue-template/webpack';
var tmpPath = path.join(home, '.wsfe-templates/webpack');
...
function init(program) {
  ...
  program.command('init [projectName]').description('generator a new project from template').option('-c, --clone', 'use git clone').action(function (rawName, cmd) {
    ...
    function generate(name, src, dest, done) {
      ...
      metalsmith.use(askQuestions(opts.prompts)).user(filterFiles(opts.filters)).clean(false).destination(dest).build(function (err, files) {
        done(err);
