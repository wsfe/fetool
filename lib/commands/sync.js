...
var Sync = function () {
  ...
  _createClass(Sync, [{
    ...
    value: function sync() {
      ...
      var _args = ['-rzcvp', process.platform === 'win32' ? "--chmod=a='rX,u+w,g+w'" : "--chmod=u+rwX,g+rwX,o+rX,o-w", "--rsync-path='" + (this.conf.sudo ? "sudo " : '') + "rsync'", this.conf.local, '' + this.conf.user + this.conf.host + ':' + this.conf.path, default_include, default_exclude];
      if (this.conf.port) {
